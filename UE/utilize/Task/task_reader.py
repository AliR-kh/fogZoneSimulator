...
class Task_reader:
    ...
    def creat_dag(self,device_id):
        ...
        xml_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../Config/Workflow/Montage_20.xml'))
        #path=os.path("Config/Workflow/Montage_40.xml")
        tree = ET.parse(xml_path)  # باز کردن فایل xml
        listjob = []
        for job in tree.iter('job'):  # \پیدا کردن تگ های job
            temp = 0
            temp = []
            sizein = 0
            sizeout=0
